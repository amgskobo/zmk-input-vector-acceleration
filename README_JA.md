...
3. 直近数レポートから counts/sec の速度を求めます。移動量とレポート間の時間を
   それぞれ合計として持ち、レポートが加わるたびに古い値を半分だけ残します。
   速度はその比です。
...
入力元とこのプロセッサーの間に通信路がある場合、1レポート分の間隔は速度の尺度に
なりません。分割キーボードのペリフェラルからのレポートはBluetoothの接続イベントで
届くため、10 msごとに送ったレポートがセントラルには7.5 ms、15 ms、あるいは1 ms未満の
間隔で届きます。1レポートずつ読むと、一定速度のストロークでも倍率が1.0倍と最大値の間で
揺れていました。合計で見れば途中のレポートの到着時刻は打ち消し合い、直前と同じ
ミリ秒に届いたレポートもそのまま合計に加わります。
...
続いて同じコマンドで、driverとcustom-settings adapterの全関数を `tests/runtime/` の
スタブ付きハーネスに取り出します。eventの振り分けとstream・listenerごとの端数、runtime API、
2つの永続化構成(driver自身が遅延work itemで保存・読み込みする構成と、custom-settingsが
永続化を持つときのRAMのみの構成)、adapterの読み出しと重複キー検査を確認します。CIは
`vector_accel_core.c` と取り出した各関数の行・分岐100%を要求します。
保存された設定をドライバ呼び出しに変える適用処理もマクロごと取り出し、ソースに
ゲートのない関数が1つでもあれば runner が失敗します。devicetree からの
インスタンス生成そのものは統合テストで確認します。
